...
chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
  ...
  chrome.tabs.onUpdated.addListener(function(_tabId, changeInfo, tab) {
    ...
    if(changeInfo.url && changeInfo.url.includes("&gather=")){
      let gatherid = getGatherIdFromUrl(changeInfo.url);
      let videoid = getVideoIdFromUrl(changeInfo.url);
      let tabId = _tabId;

      sessionMap.set(tabId, [videoid, gatherid]);
      console.log("Added session to session map: " + tabId + ": [" + videoid + ", " + gatherid + "]");
    }
  ...
  function(request, sender, sendResponse) {
    ...
    if(request.type == "urlVariableInfo" && sessionMap.has(request.tabId)) {
      ...
      sendResponse({
        ...
        gatherid: sessionMap.get(request.tabId)[1],
        tabId: request.tabId
      })
    } else {
...
/**
 * getGatherIdFromUrl returns derived gather id from url
 * @param {*} url 
 * @returns 
 */
function getGatherIdFromUrl(url) {
  ...
  for (var i = 0; i < segments.length; i++) {
      ...
      if (segment.length == 2 && segment[0] == 'gather') {
          return segment[1];
      }
  }
